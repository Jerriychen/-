...
def plot_results_with_hyperplane(clf, clf_name, df, plt_nmbr):
	x_min, x_max = df.x.min() - 0.5, df.x.max() + .5
	y_min, y_max = df.y.min() - 0.5, df.y.max() + .5
	...
	print xx.shape,yy.shape,Z.reshape

	# create a subplot (we're going to have more than 1 plot on a given image)
	pl.subplot(2, 2, plt_nmbr)
	# plot the boundaries    c=df.animal_type, label="cows" if animal=="x" else "wolves",
